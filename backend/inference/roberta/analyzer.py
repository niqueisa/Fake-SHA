"""
roberta inference: same api response shape as svm and mock.
...
from core.config import get_inference_temperature, get_roberta_model_source
...
def analyze_text(text: str, title: str = "", url: str = "") -> AnalyzeResponse:
    ...
    temperature = get_inference_temperature(model_source, backend="roberta")
    if logger.isEnabledFor(logging.DEBUG):
        logger.debug(
            "roberta model_source=%r inference_temperature=%s",
            model_source,
            temperature,
        )
    bundle = load_model()
