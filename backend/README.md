...
FastAPI backend for fake news detection. Inference lives under `inference/` (SVM + TF‑IDF today; RoBERTa later). Training scripts live under `training/` and are not imported by the API at runtime.
...
├── inference/              # Classifiers: svm/, mock/, factory.py
...
| `inference/factory.py` | Chooses analyzer via env `FAKE_SHA_ANALYZER` (`svm` default, or `mock`). |
