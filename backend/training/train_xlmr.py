"""
FAKE-SHA thesis: Fine-tune an XLM-RoBERTa / RoBERTa / compatible sequence classifier.

Aligned with :mod:`inference.roberta` / :mod:`inference.xlmr`
...
def _compute_metrics_builder():
    def compute_metrics(eval_pred):
        ...
        return {
            ...
            "f1_fake": f1_score(
                labels, preds, pos_label=0, average="binary", zero_division=0
            ),
...
def _tokenize_fn(batch: dict, tokenizer: PreTrainedTokenizerBase, max_length: int):
    ...
    # RoBERTa/XLM-R do not need token_type_ids.
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Fine-tune transformer sequence classification for FAKE-SHA."
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Hugging Face dataset ID. If set, CSV paths are ignored.",
    )
    ...
    parser.add_argument(
        ...
        help="save_pretrained output directory.",
    )
    parser.add_argument(
        "--model-name",
        type=str,
...
def main() -> None:
    ...
    train_labels = _normalize_labels(train_labels, "Train")
    val_labels = _normalize_labels(val_labels, "Validation")
    test_labels = _normalize_labels(test_labels, "Test")
    ...
    print(
        f"seed={args.seed}, article_only={args.article_only}, "
    ...
    if args.model_name.startswith("dost-asti/"):
        use_fp16 = False
        print("fp16 disabled automatically for dost-asti checkpoint.")
    ...
    print(
        "\nDone. Point your analyzer to this saved model folder "
