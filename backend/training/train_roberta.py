"""
FAKE-SHA thesis: Fine-tune a RoBERTa (or compatible) sequence classifier.
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Fine-tune RoBERTa sequence classification for FAKE-SHA."
    )
    ...
    parser.add_argument(
        ...
        help="Hugging Face save_pretrained directory (default: backend/artifacts/roberta).",
    )
    parser.add_argument(
        "--model-name",
        type=str,
        default="roberta-base",
        help="Base checkpoint (e.g. roberta-base, roberta-large).",
    )
    ...
    parser.add_argument(
        ...
        help="Use class-balanced loss weights from the training set (recommended if imbalanced).",
    )
    parser.add_argument(
        "--article-only",
        action="store_true",
...
def main() -> None:
    ...
    if args.hf_dataset:
        ...
        train_texts, train_labels = load_classification_csv(
            args.train_csv, article_only=article_only, tfidf_preprocess=False
        )
        val_texts, val_labels = load_classification_csv(
            args.val_csv, article_only=article_only, tfidf_preprocess=False
        )
        test_texts, test_labels = load_classification_csv(
            args.test_csv, article_only=article_only, tfidf_preprocess=False
        )
    ...
    print("\nDone. Point FAKE_SHA_ANALYZER=roberta at this backend (or restart) and POST /analyze.")
